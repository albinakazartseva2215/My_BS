...
| `UNIQUE(master_id, start_datetime) WHERE status IN ('hold','confirmed')` | `appointments` | это и есть защита от сценария 2 из паспорта («состояние гонки»): два клиента не могут одновременно удержать/подтвердить один и тот же мастер+время | без индекса возможна двойная запись — ровно тот риск, который паспорт прямо называет главным. Ограничение частичное (только для активных статусов), поэтому отменённые/истёкшие записи не мешают заново забронировать освободившееся время |
...
1. **Точное совпадение времени начала вместо полноценного исключения пересекающихся диапазонов.** Уникальный индекс `ux_appt_master_slot_active` защищает от буквального сценария 2 из паспорта — двух записей на один и тот же мастер и одну и ту же минуту старта. Он не ловит случай, когда услуги разной длительности создают частичное пересечение при *разных* стартах (SQLite не поддерживает range-exclusion ограничения, как `EXCLUDE` в PostgreSQL). Поэтому индекс — это последний рубеж защиты на уровне БД, а не единственная проверка: перед вставкой запись должна создаваться внутри транзакции (`BEGIN IMMEDIATE`) с явной проверкой пересечения по диапазону в коде приложения.
...
8. **`password_reset_tokens` включена в схему, хотя экрана восстановления пароля в прототипе нет.** Лист «Нерешённые случаи» явно фиксирует это как пробел в навигации, а не как отменённую функцию — сама функция прямо названа в паспорте продукта («Функция 1 … включая восстановление пароля»). Не включить таблицу сейчас — значит молча выбросить заявленное требование вместо того, чтобы явно отметить недостающий экран.
