...
| `password_hash` | TEXT | да | | хеш пароля со своей солью (scrypt, `node:crypto`, без внешних пакетов — bcrypt/argon2 не подходят: нативные сборки, риск не собраться на сервере, см. `server/README.md`, раздел 4); самого пароля в БД нет — требование 5 |
