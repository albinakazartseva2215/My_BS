...
19. **Вход через Яндекс в один клик — `password_hash` стал необязательным, `provider`/`provider_id` добавлены в `users`; привязка по email, не по внутреннему id; заглушка вместо реального Яндекса.** Требование — кнопка «Войти через Яндекс» рядом с обычной формой, без сбора пароля. Несколько решений внутри одной миграции (`009_yandex_oauth.sql`), каждое — отдельный компромисс:
    ...
    - **Приложение в Яндексе пока не регистрируется — заглушка вместо реального обмена кода на профиль.** У сервиса ещё нет постоянного адреса, на который Яндекс мог бы вернуть пользователя (`redirect_uri`). `getYandexProfile()` (`domain/yandexAuth.js`) — единственная точка входа: при включённом `YANDEX_LOGIN_STUB_ENABLED` возвращает тестовые email/имя из `.env` (`YANDEX_LOGIN_STUB_EMAIL`/`_NAME`), иначе вызывает `fetchRealYandexProfile()` — пока не реализована, бросает понятную `501`. После регистрации приложения на oauth.yandex.ru заменяется только тело этой функции (обмен `code` на токен + запрос `login.yandex.ru/info`), остальная логика входа (`findOrCreateYandexUser`, сам роут) не меняется. Заглушка по умолчанию выключена и жёстко запрещена в `NODE_ENV=production` — `config/env.js` бросает исключение при старте, если оба условия совпали, тем же приёмом fail-fast, что уже применяется к версии Node (`db/connection.js`).
