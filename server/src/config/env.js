...
export const env = {
  ...
  // Приложение на oauth.yandex.ru (domain/yandexAuth.js, routes/auth.routes.js —
  // /api/auth/yandex/start и /yandex/callback). Секрет и идентификатор —
  // только из окружения, без значений по умолчанию (в т.ч. без пустой
  // строки — undefined честно показывает "не задано", а не маскируется под
  // рабочее значение) — задаются в .env на сервере, см. .env.example.
  yandex: {
